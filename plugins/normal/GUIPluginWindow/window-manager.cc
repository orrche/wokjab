...
ConnWindowManager::ConnWindowManager(WLSignal *wls) : WoklibPlugin(wls)
{
	...
	winid = 0;
	EXP_SIGHOOK("GUIPluginWindow Open", &ConnWindowManager::Open, 2);
	EXP_SIGHOOK("GUIPluginWindow Close", &ConnWindowManager::Close, 2);
...
{
	std::map<std::string, GUIPluginWindow*>::iterator iter;
...
ConnWindowManager::Open(WokXMLTag *tag)
{
	...
	win[str.str()] = new GUIPluginWindow(wls, str.str());
