...
GUIConnectWindow::GUIConnectWindow (WLSignal * wls, std::string pid) : WLSignalInstance(wls)
{
		id = pid;
...
{
	WokXMLTag deletetag("delete");
	deletetag.AddAttr("id", c->id);
	c->wls->SendSignal("GUIConnWindow Close", deletetag);
