...
GUIWindow::AddWidget(WokXMLTag *tag)
{
	...
	int pos = atoi(tag->GetFirstTag("widget").GetAttr("pos").c_str());
	std::vector<wids>::iterator iter;
	unsigned int n = 0;
	...
	for(iter = Widgets.begin() ; iter < Widgets.end() ; iter++)
	{
		if( iter->pos > pos )
		{
			Widgets.insert(iter, wids(atoi(tag->GetFirstTag("widget").GetAttr("id").c_str()), pos));
			gtk_box_reorder_child (GTK_BOX(glade_xml_get_widget(xml,"mainvbox")), sockwid, n);
	...
	if ( n == Widgets.size() )
	{
		Widgets.push_back(wids(atoi(tag->GetFirstTag("widget").GetAttr("id").c_str()), pos));
	}
	if ( Widgets.empty() )
		Widgets.push_back(wids(atoi(tag->GetFirstTag("widget").GetAttr("id").c_str()), pos));
...
{
	WokXMLTag nulltag("open");
	wls->SendSignal("GUIConnWindow Open", nulltag);
...
{
	WokXMLTag nulltag("open");
	wls->SendSignal("GUIPluginWindow Open", nulltag);
